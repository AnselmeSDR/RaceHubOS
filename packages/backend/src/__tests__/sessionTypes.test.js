...
describe('session types', () => {
  it('is stored as plain text, never as a database enum', () => {
    const schema = fs.readFileSync(path.join(rootDir, 'packages/backend/prisma/schema.prisma'), 'utf-8');

    // A Prisma enum makes an unexpected value unreadable: one row holding a type
    // the schema does not know and findMany() throws, so not a single session can
    // be read. On a race PC that is the whole application down. Plain text keeps
    // the data readable; the shared module validates what gets written.
    expect(schema).not.toMatch(/^enum\s/m);
    expect(schema).toMatch(/^\s*type\s+String\s+\/\//m);
  });

  it('lists its values beside the field, matching the shared module', () => {
    const schema = fs.readFileSync(path.join(rootDir, 'packages/backend/prisma/schema.prisma'), 'utf-8');
    const line = schema.split('\n').find((l) => /^\s*type\s+String\s+\/\//.test(l));

    const documented = line.split('//')[1].split('—')[0].split('|').map((s) => s.trim());
    expect(documented.sort()).toEqual([...SESSION_TYPES].sort());
  });
